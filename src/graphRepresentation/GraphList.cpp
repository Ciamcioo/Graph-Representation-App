 ...
 */
void GraphList::add_edge(int u, int v, int weight,bool directed) {
 ...
 */
void GraphList::print_list() const {
    for (int i = 0; i < vertices; ++i){
        std::cout << i << ":";
        Node* currentNode = adj_list[i];
        while (currentNode != nullptr) {
 ...
 */
int GraphList::min_key(int* key, bool* mst_set) const  {
 ...
 */
void GraphList::print_mst(int* parent) const {
 ...
 */
void  GraphList::kruskla_mst()  const {
...
void GraphList::dijkstra(int src) const { 
